...
$manager->setCredentials('localhost', 'root', 'admin', $database);
DB::init($manager);
$tables = DB::getColumn("SHOW TABLES from ".$database );
set_time_limit(0);
if($_POST['method'] == 'showTables') {
    echo json_encode($tables);
    exit;
}

if($_POST['method'] == 'searchColumn' || $_POST['method'] == 'searchValue') {
    foreach ($tables as $table) {
        if ($_POST['method'] == 'searchColumn') {
            ...
            foreach ($cols as $col) {
                ...
                $query = "SELECT count(1) FROM " . $table . " WHERE binary " . $c . "='" . addslashes($_POST['value']) . "'";
                $res = DB::getCell($query);
                if (!empty($res)) {
                    echo "select aes_decrypt(" . $c . ", '6e3b4165386d61752c31696f7e'), " . $c . " from " . $table . "/* look column: " . $c .' - '.$query.$res. "*/\n";
                    flush();
                }
            }
        }
...
if($_POST['method'] === 'query') {
    $table = $_POST['table'];
    $where = getIds();
	$query = empty($_POST['directQuery']) ? prepareQuery($table, $where) : $_POST['directQuery'];
    echo json_encode(DB::getTable($query));
}

function prepareQuery($table, $where){
    global $database;
    ...
    $dateTimeStr ='{"ppxray_billing_notes":["created_on"],"ppxray_facilityinvoice_notes":["created_date"],"ppxray_corporate_docs":["created_dt"],"ppxray_corporate_service":["created_dt"],"ppxray_equipment_accesslog":["check_in_dt","check_out_dt"],"ppxray_equipment_docs":["created_dt"],"ppxray_equipment_maintenance":["created_on"],"ppxray_facility_docs":["created_dt"],"ppxray_facility_service":["created_dt"],"ppxray_fax_deamon":["created_dt"],"ppxray_filelog":["created_dt"],"ppxray_login_report":["login_time","logout_time"],"ppxray_order":["sheduleservdate","duedate","faxed_time","created_dt"],"ppxray_order_auditlog":["action_at"],"ppxray_order_comment":["created_on"],"ppxray_order_images":["created_on"],"ppxray_order_notes":["created_on"],"ppxray_order_payment":["created_dt"],"ppxray_order_reportfax":["created_on"],"ppxray_order_reportphone":["created_on"],"ppxray_order_result":["created_on"],"ppxray_orderflow":["orderassigned_date","ordertaken_date","ordercompletion_date"],"ppxray_patient_sms":["created_tm","send_tm"],"ppxray_radiologist_group":["created_dt"],"ppxray_radiologygroup_docs":["created_date"],"ppxray_service":["created_dt"],"ppxray_service_group":["created_dt"],"ppxray_template":["created_on"],"ppxray_userinfo":["lock_timestamp","last_login","last_logout"],"ppxray_userpermission":["enable_dt","disable_dt"]}';
    $query = 'show columns from '.$table;
    $columns = DB::getTable($query);
    $decodeArray = json_decode($encodeStr, true);
    $decodeDate = json_decode($encodeStr, true);
    $iterateOver = (!empty($decodeArray[$table]) ? $decodeArray[$table] : array()) +
        (!empty($decodeDate[$table]) ? $decodeDate[$table] : array());
    $body = array();
    foreach ($columns as $col) {
        $field = strtolower($col['Field']);
        if(in_array($field, $iterateOver)) {
            $body[] =  "aes_decrypt(" . $field . ", '6e3b4165386d61752c31696f7e') as ".$field.' ';
        } else {
            $body[] = $field; 
        }
    }
    
    return 'select '.implode(',', $body).' from '.$table.' '.$where.' limit 0, 100';
}
...
function getIds(){
    ...
    return 'where refid in ('.implode(',', $out).')';
}